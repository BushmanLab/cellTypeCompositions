...
#+begin_src R
  ...
  lambdaRhoComp <- t((1.0-rowSums(om)) * with(tmp,eta[,1:etaM])) * lambdaN0
  ...
  ## list(t(etaVisTab),round(lambdaRhoComp))

  etaTab <- etaVisTab + t( etaInvisTab )

  etaUpdate <- prop.table(array(rgamma(etaTab,etaTab+1.0),dim(etaTab)),2)

  ## round(cbind(with(tmp,t(eta[,1:etaM])),t(etaUpdate)),3)

  ## round(with(tmp,t(eta[,1:etaM]))-t(etaUpdate),3)
  ...
  for (eta_indx in 1:nrow(etaLambdaTab)){
    ...
    for (j in 1:ncol(etaLambdaTab)){
      if (etaLambdaTab[eta_indx,j] != 0L)
      lambdaN0[eta_indx,j] <- 
	rhoplus_elt * rN0GivenN1(etaLambdaTab[eta_indx,j],tmp$lambda[j], rhoplus_elt)
