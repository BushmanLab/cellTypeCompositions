...
#+begin_src R
  ...
  tuneScan <- function(data, uop, scan, nreps=50L,
		       dprior=1.0, lambdaAlpha=1.0, lambdaBeta=0.01){
  
    etaLambdaTab <- with(scan,table(dataToEta,dataToLambda))
    lambdaPostTab <- xtabs(rowSums(with(data,tab[data.index,]))~ scan$dataToLambda)
    etaPostTab <- xtabs(with(data,tab[data.index,])~ scan$dataToEta)
  ...
  res <- tuneScan(wtab,om,tmp0)
  res2 <-   tuneScan(wtab,om,res)
  plot(llk)
