  ...
  res <- 
    with(tmp,
	 sampleParms(wtab$tab, wtab$data.index-1L, om, dataToEta-1L, dataToLambda-1L,
		     eta, etaM, lambda, lambdaN, lambdaM, dprior=1.0,
		     lambdaAlpha=1.0, lambdaBeta=0.01, dpriorLambda=1.0,
		     verbose = 1L))
  ...
  system.time(for (i in 1:100){
    res <- with(tmp,
		sampleParms(wtab$tab, wtab$data.index-1L, om,
			    ...
			    res$eta, etaM, res$lambda, lambdaN, lambdaM, dprior=1.0,
			    lambdaAlpha=1.0, lambdaBeta=0.01, dpriorLambda=1.0,niter=20L,
			    verbose = 1L))
    llk <- c(llk,sum(ept*log(prop.table(t(res$eta)%*%om,1))))
    if (anyNA(res)) break
    res2 <- res
  })
...
#+begin_src cpp
  ...
  List sampleParms(
		   ...
		   double dpriorLambda, int niter = 5L, int verbose=0L){
    ...
    return List::create(_["eta"] = eta.head_cols(etaM),
			_["lambda"] = lambda.head(lambdaM));
  }
...
#+begin_src cpp
    ...
    // eta.by.ct aka etaPostTab in R
    imat eta_by_ct( etaM, J , fill::zeros );
    // eta.by.lambda.by.r aka etaLambdaTab in R
    imat eta_by_lambda_tab(etaM, lambdaM, fill::zeros );
    // lambda_post_tab
    imat lambda_post_tab(lambdaM, J, fill::zeroes );

    for (int i = 0L; i<ndat; i++){
      eta_by_ct.row( dataToEta(i))  += tab.row( di(i) );
      lambda_post_tab.row( dataToLambda(i)) +=  tab.row( di(i) );
      eta_by_lambda_tab( dataToEta(i) , dataToLambda(i) )++ );
    }
  ...
  // R_minus_r
	...
	R_minus_r(i, j) = (eta_by_lambda_tab(i,j) == 0L) ? 0.0 :
	  Rf_rnbinom(( double) eta_by_lambda_tab(i,j), rhoTilde(i,j));
  ...
  // R
    mat R = R_minus_r + eta_by_lambda_tab;
